...
export const Scene7Line3: React.FC = () => {
    ...
    const exitFrame = SCENE7_LINE3_DUR - 12;
    const exitProgress = interpolate(frame, [exitFrame, SCENE7_LINE3_DUR], [0, 1], { extrapolateLeft: "clamp" });
    const contentOpacity = interpolate(exitProgress, [0, 1], [1, 0]);

    return (
        <Scene4Wrapper
            duration={SCENE7_LINE3_DUR}
            bg="transparent"
        ...
        >
            ...
            >
                <div className="flex flex-col items-center gap-6">
                    <DocumentReveal
                        text="Unopinda muDashboard yako yeCarbarn"
                        mode="word"
                        delay={10}
                        className="text-6xl text-slate-800 font-bold text-center"
                        highlightWords={["Dashboard", "Carbarn"]}
                        highlightColor={WARNING_RED}
                    />
                    <DocumentReveal
                        text="kuti utevere kufamba kwechikepe,"
                        mode="word"
                        delay={40}
                        className="text-5xl font-black text-slate-800 text-center"
                        highlightWords={["utevere", "chikepe"]}
                        highlightColor={TECH_BLUE}
                    />
                    <DocumentReveal
                        text="kudhawnilodha magwaro"
                        mode="word"
                        delay={70}
                        className="text-5xl font-black text-slate-800 text-center"
                        highlightWords={["magwaro"]}
                        highlightColor={TECH_BLUE}
                    />
                </div>
